...
import java.util.Scanner;
	...
	public class JVPrincipal {
			...
			private static void iniciarSesionCorrecta(){
			//bucle de credenciales de entrada
			Scanner teclado2 = new Scanner(System.in);
			Usuario usrAux; 
			Date fecha = new Date();
			do{
				System.out.println("Introduzca su nif: ");
				nifusuario = teclado2.nextLine();
				usrAux = buscarUsuario(nifusuario);
				System.out.println("Ahora introduzca su clave:");
				claveusuario = teclado2.nextLine();
					if(usrAux != null){
						if(usrAux.claveAcceso.equals(claveusuario)){
							SesionUsuario sesion = new SesionUsuario();
							sesion.usr=usrAux;
							sesion.fecha=fecha.toString();
							datosSesiones[9] = sesion;
							break;
						}else{
						...
						System.out.println("ERROR. No hay ningun usuario llamado asi en este sistema");
					continue;
					}
			}while(ejecuciones < intentos);
				if(ejecuciones == 3){
				System.out.println("Se ha superado el número de intentos");}
				
				System.out.println("Bienvenido al sistema: " +usrAux.nombre);
				System.out.println(fecha.toString());	
				//arrancarSimulacion()
				
}//iniciarSesionCorrecta
			private static Usuario buscarUsuario(String nifusuario){
				for(int i = 0; i < MAX_USUARIOS;i++){
					if(nifusuario.equals(datosUsuarios[i].nif)){
						int o = 1;
